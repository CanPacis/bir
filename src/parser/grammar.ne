...
BlockDeclarationStatement -> identifier BlockVerb:* __ "[" _ (ArgumentList _ {% id %}):? "]" _ BlockContent
	{% d => ({ operation: "block_declaration", name: d[0], verbs: d[1], arguments: d[5], body: d[8], position: d[0].position, implementing: false }) %}
  | identifier _ "implements" _ identifier 
  {% d => ({ operation: "block_declaration", name: d[0], implements: d[4], position: d[0].position, implementing: true }) %}
